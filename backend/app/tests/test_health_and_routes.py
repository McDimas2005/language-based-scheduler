...
async def test_health_route():
    ...
    assert "whisper" in body["models"]
    assert body["calendar"]["optional"] is True
...
def make_calendar_disabled_settings(tmp_path):
    return Settings(
...
@pytest.mark.anyio
async def test_google_status_unconfigured_is_optional(tmp_path, monkeypatch):
    settings = make_calendar_disabled_settings(tmp_path)
    services = app.state.services
    monkeypatch.setattr(services, "settings", settings)
    monkeypatch.setattr(services, "calendar_service", CalendarService(settings))
    ...
    assert body["message"] == "Google Calendar is not configured for this deployment."
...
@pytest.mark.anyio
async def test_calendar_create_unconfigured_is_friendly(tmp_path, monkeypatch):
    settings = make_calendar_disabled_settings(tmp_path)
    services = app.state.services
    monkeypatch.setattr(services, "settings", settings)
    monkeypatch.setattr(services, "calendar_service", CalendarService(settings))
    ...
    assert "Event draft generation is still available" in response.json()["detail"]
